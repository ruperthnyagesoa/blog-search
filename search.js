...
function searchtext() {
    ...
    if (input.value == '') {
        result.innerHTML = '<i>- search -</i><hr>'+'<p align="center">Input the keywords in the artivle search and the text query</p><hr>'
    }
        ...
        for (i = 0; i < resulttitlecache.length; i++) {
            for (j = 0; j < obj.articles.length; j++) {
                if (obj.articles[j]['title'] == resulttitlecache[i]) {
                    titlesearchresult = '<h4><a href="'+obj.articles[j]["path"]+'" class="resulttitle">'+obj.articles[j]['title'].replace(new RegExp(input.value, 'g'), '<mark>'+input.value+'</mark>')+'</a></h4><em>-search matching</em><p class="showbox">'+obj.articles[j]['text'].substring(0, 100)+'</p>'
    ...
    // The body of the results content
    ...
    // integration
    result.innerHTML = result.innerHTML.substring(0, result.innerHTML.length-4)+textsearchresult.substring(0, textsearchresult.length-4)+'<hr><a href="https://github.com/ruperthnyagesoa/blog-search" class="tr">Search powered by Ruperth\'s blog-search</a>'
}
